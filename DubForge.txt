...
T = {
    ...
    "btn_stop":     ("Stopp", "Stop"),
    "mute_vocals":  ("Stimmen stumm", "Mute Vocals"),
    "mute_bgm":     ("BGM/SFX stumm", "Mute BGM/SFX"),
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.backing_path = None     # alles ausser Stimmen / everything else (BGM, SFX, Music)
        self.wave_data = []          # Vocals Wellenform
        self.wave_sr = 8000
        self.bgm_wave_data = []      # BGM Wellenform
        self.bgm_wave_sr = 8000
        self.duration = 0.0
        ...
        self._bgm_peak_cache = (None, None)
        self._syncing = False
    ...
    def _init_vars(self):
        ...
        self.mute_vocals = tk.BooleanVar(value=False)
        self.mute_bgm = tk.BooleanVar(value=False)
    ...
    def _build_ui(self):
        ...
        self.canvas = tk.Canvas(step2, height=210, bg="#15161c",
        ...
        # Mute Toggles for Dual-Stem Vocals & BGM Auditioning
        mframe = ttk.Frame(side)
        mframe.pack(fill="x", pady=(4, 6))
        ttk.Checkbutton(mframe, text=t("mute_vocals"), variable=self.mute_vocals,
                        command=self._on_mute_change).pack(anchor="w", pady=1)
        ttk.Checkbutton(mframe, text=t("mute_bgm"), variable=self.mute_bgm,
                        command=self._on_mute_change).pack(anchor="w", pady=1)

        for key, cmd in (("btn_rename", self._rename_selected),
    ...
    def _do_analyze(self, src, mode, t0, t1, separate):
        ...
        if separate:
            ...
            try:
                # Main model: htdemucs_ft splits video audio into Vocals & BGM (SFX + Music)
                voc, nov = pc.separate_vocals(self.audio_path, self.work,
                                              log=self._log, model="htdemucs_ft")
        ...
        if self.backing_path and os.path.isfile(self.backing_path):
            self.bgm_wave_data, self.bgm_wave_sr = pc.load_mono(self.backing_path)
        else:
            self.bgm_wave_data = []
            self.bgm_wave_sr = self.wave_sr
        self._bgm_peak_cache = (None, None)

        self._set_status(t("st_detect"), 88)
        # Use Silero Voice Activity Detector (Silero VAD) to clip vocals timeline
    ...
    def _on_mute_change(self):
        self.draw_wave()
        if getattr(self, "_is_sd_playing", False):
            self._play_selected()
    ...
    def _play_selected(self):
        ...
        mute_voc = bool(self.mute_vocals.get())
        mute_bgm = bool(self.mute_bgm.get())
        if mute_voc and mute_bgm:
            return  # Beide stummgeschaltet -> keine Wiedergabe

        start_s = float(c["start"])
        dur_s = float(c["end"] - c["start"])
        if dur_s <= 0.005:
            return
        ...
        try:
            ...
            voc_chunk = None
            sr = 44100
            if not mute_voc and voc_src and os.path.isfile(voc_src):
                with sf.SoundFile(voc_src) as f:
                    sr = f.samplerate
                    start_f = max(0, int(start_s * sr))
                    num_f = max(1, int(dur_s * sr))
                    if start_f < len(f):
                        f.seek(start_f)
                        voc_chunk = f.read(num_f, dtype="float32")

            bgm_chunk = None
            if not mute_bgm and bgm_src and os.path.isfile(bgm_src):
                with sf.SoundFile(bgm_src) as f:
                    sr_bgm = f.samplerate
                    start_f = max(0, int(start_s * sr_bgm))
                    num_f = max(1, int(dur_s * sr_bgm))
                    if start_f < len(f):
                        f.seek(start_f)
                        bgm_chunk = f.read(num_f, dtype="float32")

            out = None
            if mute_bgm and voc_chunk is not None:
                out = voc_chunk
            elif mute_voc and bgm_chunk is not None:
                out = bgm_chunk
            elif voc_chunk is not None and bgm_chunk is not None:
                if voc_chunk.ndim > 1: voc_chunk = voc_chunk.mean(axis=1)
                if bgm_chunk.ndim > 1: bgm_chunk = bgm_chunk.mean(axis=1)
                min_len = min(len(voc_chunk), len(bgm_chunk))
                out = (voc_chunk[:min_len] * 0.95 + bgm_chunk[:min_len] * 0.85).clip(-1.0, 1.0)
            elif voc_chunk is not None:
                out = voc_chunk
            elif bgm_chunk is not None:
                out = bgm_chunk

            if out is not None and len(out) > 0:
                sd.play(out, samplerate=sr)
                self._is_sd_playing = True
                return
        except Exception as ex:
            self._log(t("log_noplay", ex))

        # Fallback ffplay
        src = voc_src if not mute_voc else (bgm_src or voc_src)
        fp = pc.ffplay()
        try:
            if fp:
    ...
    def draw_wave(self):
        ...
        span = max(1e-6, self.view_b - self.view_a)
        has_bgm = bool(len(self.bgm_wave_data))

        n_voc = len(self.wave_data)
        a_voc = int(self.view_a / self.duration * n_voc)
        b_voc = int(self.view_b / self.duration * n_voc)
        b_voc = max(a_voc + 2, min(n_voc, b_voc))
        key_voc = (a_voc, b_voc, w)
        if self._peak_cache[0] == key_voc:
            voc_peaks = self._peak_cache[1]
        else:
            voc_peaks = pc.waveform_peaks(self.wave_data[a_voc:b_voc], w)
            self._peak_cache = (key_voc, voc_peaks)

        if has_bgm:
            # --- DUAL-STEM TIMELINE (Vocals & BGM/SFX getrennte Wellenformen) ---
            top_y0 = 4
            top_y1 = int(h * 0.49)
            mid_voc = (top_y0 + top_y1) / 2.0
            div_y = int(h * 0.50)
            bot_y0 = div_y + 2
            bot_y1 = h - 18
            mid_bgm = (bot_y0 + bot_y1) / 2.0

            n_bgm = len(self.bgm_wave_data)
            a_bgm = int(self.view_a / self.duration * n_bgm)
            b_bgm = int(self.view_b / self.duration * n_bgm)
            b_bgm = max(a_bgm + 2, min(n_bgm, b_bgm))
            key_bgm = (a_bgm, b_bgm, w)
            if self._bgm_peak_cache[0] == key_bgm:
                bgm_peaks = self._bgm_peak_cache[1]
            else:
                bgm_peaks = pc.waveform_peaks(self.bgm_wave_data[a_bgm:b_bgm], w)
                self._bgm_peak_cache = (key_bgm, bgm_peaks)

            # 1. Clips in der oberen Vocals-Spur
            for i, c in enumerate(self.clips):
                if c["end"] < self.view_a or c["start"] > self.view_b:
                    continue
                x0, x1 = self._t2x(c["start"]), self._t2x(c["end"])
                sel = (i == self.selected)
                cv.create_rectangle(x0, top_y0, x1, top_y1,
                                    fill=CLIP_SEL if sel else CLIP_FILL,
                                    outline=ACC2 if sel else "#4a628a",
                                    width=2 if sel else 1)
                if x1 - x0 > 30:
                    cv.create_text(x0 + 5, top_y0 + 10, anchor="w", fill="#dfe4f5",
                                   text="%d" % (i + 1), font=("Segoe UI Semibold", 8))

            # 2. Vocals Wellenform zeichnen
            voc_muted = bool(self.mute_vocals.get())
            voc_color = "#3a4055" if voc_muted else "#38bdf8"
            scale_voc = max(1.0, (top_y1 - top_y0) / 2.0 - 4)
            for x, (lo, hi) in enumerate(voc_peaks):
                y0 = mid_voc - hi * scale_voc
                y1 = mid_voc - lo * scale_voc
                if abs(y1 - y0) < 1: y1 = y0 + 1
                cv.create_line(x, y0, x, y1, fill=voc_color)
            cv.create_line(0, mid_voc, w, mid_voc, fill="#232635")

            # Vocals Spur-Badge
            voc_lbl = "🎙️ VOCALS / DIALOGUE (htdemucs_ft)" + ("  [MUTED]" if voc_muted else "")
            cv.create_text(8, top_y0 + 7, anchor="w",
                           fill="#e06c75" if voc_muted else "#7dd3fc",
                           text=voc_lbl, font=("Segoe UI Bold", 8))

            # Spur-Trennlinie
            cv.create_line(0, div_y, w, div_y, fill="#2d3142", width=2)

            # 3. BGM / SFX Wellenform zeichnen
            bgm_muted = bool(self.mute_bgm.get())
            bgm_color = "#3a3045" if bgm_muted else "#c084fc"
            scale_bgm = max(1.0, (bot_y1 - bot_y0) / 2.0 - 4)
            for x, (lo, hi) in enumerate(bgm_peaks):
                y0 = mid_bgm - hi * scale_bgm
                y1 = mid_bgm - lo * scale_bgm
                if abs(y1 - y0) < 1: y1 = y0 + 1
                cv.create_line(x, y0, x, y1, fill=bgm_color)
            cv.create_line(0, mid_bgm, w, mid_bgm, fill="#232635")

            # BGM Spur-Badge
            bgm_lbl = "🎵 BGM / INSTRUMENTAL & SFX (htdemucs_ft)" + ("  [MUTED]" if bgm_muted else "")
            cv.create_text(8, bot_y0 + 7, anchor="w",
                           fill="#e06c75" if bgm_muted else "#d8b4fe",
                           text=bgm_lbl, font=("Segoe UI Bold", 8))

        else:
            # Single-Track Anzeige wenn noch nicht getrennt
            for i, c in enumerate(self.clips):
                if c["end"] < self.view_a or c["start"] > self.view_b:
                    continue
                x0, x1 = self._t2x(c["start"]), self._t2x(c["end"])
