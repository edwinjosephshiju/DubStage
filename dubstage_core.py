...
def make_beep(duration=0.08, frequency=880.0, volume=0.22):
    """Generate a short click/beep with fades to avoid audible clicks."""
    n = max(1, int(float(duration) * SR))
    t = np.arange(n, dtype=np.float32) / float(SR)
    audio = np.sin(2.0 * np.pi * float(frequency) * t)
...
def make_countdown_tick():
    """Quiet countdown tick."""
    return make_beep(0.055, 720.0, 0.16)


def make_start_cue():
    """Distinct two-tone cue indicating that recording is about to start."""
    a = make_beep(0.075, 880.0, 0.22)
    b = make_beep(0.12, 1320.0, 0.28)
    gap = np.zeros(int(0.025 * SR), dtype=np.float32)
    return np.concatenate((a, gap, b)).astype(np.float32)
...
class Mic(object):
    ...
    def prime(self, device=None):
        """Open and start the input stream without beginning a take."""
        if not self.sd:
        ...
        def cb(indata, frames, time_info, status):
            ...
            buf = data if not len(self._buf) else np.concatenate([self._buf, data])
            step = self._env_step
            k = len(buf) // step
            if k:
    ...
    def start(self, playback=None, device=None):
        """Start a fresh take, optionally with simultaneous backing playback."""
        if not self.sd:
            raise RuntimeError("sounddevice fehlt / missing")
        self._chunks = []
        self._env = []
