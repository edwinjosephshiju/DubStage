...
def separate_vocals(wav_path, workdir, log=None, model="htdemucs_ft"):
    """Gibt (vocals_wav, no_vocals_wav) zurueck unter Verwendung von Demucs (Standard: htdemucs_ft)."""
...
# --------------------------------------------------------------------------
# Silero Voice Activity Detector (Silero VAD)
# --------------------------------------------------------------------------

_SILERO_MODEL = None

def get_silero_vad():
    """Laedt das neuronale Silero-VAD-Modell (gecached oder via Torch Hub)."""
    ...
    try:
        ...
        candidates = [
            ...
            os.path.join(os.path.dirname(__file__), "silero_vad.jit"),
        ]
        try:
            ...
            if os.path.isdir(hub_dir):
                for root, _dirs, files in os.walk(hub_dir):
                    if "silero_vad.jit" in files:
                        candidates.append(os.path.join(root, "silero_vad.jit"))
                        break
        except Exception:
            pass
        ...
        try:
            m, _ = torch.hub.load(repo_or_dir="snakers4/silero-vad", model="silero_vad", onnx=False, trust_repo=True)
            m.eval()
            _SILERO_MODEL = m
            return _SILERO_MODEL
        except Exception:
...
def detect_clips_silero(data, sr, min_silence=0.35, min_clip=0.30, max_clip=6.0,
                        pad=0.08, sensitivity=1.0, log=None):
    """
    Silero Voice Activity Detector (Silero VAD).
    Verwendet das Silero-Neuronalnetz zur sub-millisekundengenauen
    Sprechabschnitt-Erkennung aus der Vocals-Spur.
    """
    model = get_silero_vad()
    if model is None:
        if log:
            log("Silero VAD nicht verfuegbar - nutze RMS-Detektor")
        return detect_clips_rms(data, sr, min_silence=min_silence, min_clip=min_clip,
                                max_clip=max_clip, pad=pad, sensitivity=sensitivity)
    ...
    try:
        ...
        # Schnelles Resampling auf 16 kHz (optimale Silero-Aufloesung)
        if sr != target_sr:
            num_target = int(len(arr) * target_sr / float(sr))
            if num_target == 0:
                return []
            indices = np.linspace(0, len(arr) - 1, num_target)
            audio_16k = np.interp(indices, np.arange(len(arr)), arr).astype(np.float32)
        else:
            audio_16k = arr

        window_size = 512
        num_windows = len(audio_16k) // window_size
        if num_windows == 0:
            return []
        ...
        with torch.no_grad():
            for i in range(num_windows):
                ...
                try:
                    prob = float(model(chunk, target_sr).item())
                except Exception:
                    prob = 0.0
                probs.append(prob)

        win_dur = window_size / float(target_sr) # 0.032s
        thresh = float(np.clip(0.40 / max(0.2, sensitivity), 0.15, 0.80))
        neg_thresh = thresh * 0.65

        # Silero Hysterese-Zustandsautomat fuer Sprechgrenzen
        segs = []
        is_speech = False
        start_win = 0
        silence_win_count = 0
        need_silence_win = max(1, int(min_silence / win_dur))
        ...
        for w, prob in enumerate(probs):
            if not is_speech:
                ...
                if prob < neg_thresh:
                    silence_win_count += 1
                    if silence_win_count >= need_silence_win:
                        end_win = max(start_win + 1, w - silence_win_count + 1)
                        segs.append((start_win, end_win))
                        is_speech = False
                        silence_win_count = 0
                else:
                    silence_win_count = 0
        ...
        if is_speech:
            segs.append((start_win, num_windows))

        total_dur = len(audio_16k) / float(target_sr)
        raw_clips = []
        for a, b in segs:
            s = max(0.0, a * win_dur - pad)
            e = min(total_dur, b * win_dur + pad)
            if e - s < min_clip:
                continue
            raw_clips.append((s, e))

        # Benachbarte Abschnitte zusammenfuegen, wenn Pause kleiner als min_silence
        merged = []
        ...
        out = []
        for s, e in merged:
            out.extend(_split_long_silero(s, e, probs, win_dur, max_clip, min_clip))

        if not out and total_dur > 1.0:
            return detect_clips_rms(data, sr, min_silence=min_silence, min_clip=min_clip,
                                    max_clip=max_clip, pad=pad, sensitivity=sensitivity)

        if log:
            log(f"Silero VAD: {len(out)} Clips isoliert")
        return out
    except Exception as ex:
        if log:
            log(f"Silero VAD Hinweis: {ex} - nutze RMS-Detektor")
        return detect_clips_rms(data, sr, min_silence=min_silence, min_clip=min_clip,
                                max_clip=max_clip, pad=pad, sensitivity=sensitivity)


def _split_long_silero(s, e, probs, win_dur, max_clip, min_clip):
    """Zerlegt lange Segmente an natuerlichen Sprechpausen (Silero-Prob-Minima)."""
    if e - s <= max_clip:
        return [(s, e)]
    a = int(s / win_dur)
    b = int(e / win_dur)
    lo = a + int((b - a) * 0.30)
    hi = a + int((b - a) * 0.70)
    if hi <= lo or lo >= len(probs):
        mid = (s + e) / 2.0
        return _split_long_silero(s, mid, probs, win_dur, max_clip, min_clip) + \
               _split_long_silero(mid, e, probs, win_dur, max_clip, min_clip)
    window = probs[lo:min(hi, len(probs))]
    if not window:
        mid = (s + e) / 2.0
        return [(s, mid), (mid, e)]
    ...
    if cut_s - s < min_clip or e - cut_s < min_clip:
        return [(s, e)]
    return _split_long_silero(s, cut_s, probs, win_dur, max_clip, min_clip) + \
           _split_long_silero(cut_s, e, probs, win_dur, max_clip, min_clip)


def detect_clips_rms(data, sr, min_silence=0.35, min_clip=0.30, max_clip=6.0,
                     pad=0.08, sensitivity=1.0):
    """RMS-Energie basierter Fallback-Detektor."""
...
def detect_clips(data, sr, min_silence=0.35, min_clip=0.30, max_clip=6.0,
                 pad=0.08, sensitivity=1.0, log=None):
    """
    Findet Sprechabschnitte. Verwendet Silero VAD (mit RMS-Fallback).
    Gibt Liste von (start_s, end_s) zurueck.
    """
    return detect_clips_silero(data, sr, min_silence=min_silence, min_clip=min_clip,
                               max_clip=max_clip, pad=pad, sensitivity=sensitivity, log=log)
