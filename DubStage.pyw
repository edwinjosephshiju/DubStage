...
TAIL = 1.0            # Nachlauf der Aufnahme / recording tail in seconds

# Countdown timing is based on perf_counter() so Tkinter timer jitter
# cannot accumulate into the recording start time.
COUNTDOWN_DURATION = 3.0
GO_DELAY = 1.0          # GO remains visible for one second before recording.
...
class Game(tk.Tk):
    ...
    def _force_idle(self):
        ...
        for job_name in ("_countdown_job", "_go_job"):
        ...
        try:
            if self.screen == "stage":
                self._overlay(None)
                if self.countdown_bar_bg is not None:
                    self.cv.itemconfigure(self.countdown_bar_bg, state="hidden")
                if self.countdown_bar is not None:
                    self.cv.itemconfigure(self.countdown_bar, state="hidden")
                self.sync()
            elif self.screen == "finale":
    ...
    def do_record(self):
        ...
        # Cancel any stale countdown callbacks from a previous attempt.
        for job_name in ("_countdown_job", "_go_job"):
            job = getattr(self, job_name, None)
            if job is not None:
                try:
        ...
        # Include the one-second GO hold in the phase deadline.
        self._set_phase(
            "countdown",
            COUNTDOWN_DURATION + GO_DELAY
        )

        try:
            self._overlay("3", GOLD, 76)
            self._draw_countdown_progress(0.0)
            self.sync()
        except Exception:
            traceback.print_exc()

        # The callback only drives the display. The real clock is t0.
        self._countdown_job = self.after(0, self._countdown)

    def _countdown(self):
        if self.phase != "countdown" or self._countdown_t0 is None:
            return

        now = time.perf_counter()
        elapsed = now - self._countdown_t0
        duration = self._countdown_duration

        # Keep the preview on the first frame during the countdown.
        try:
            line = self.current_line()
            if line is not None:
                self.show_frame(line.start)
        ...
        try:
            if number is not None:
                self._overlay(number, GOLD, 76)

                # One short cue when each number becomes active.
                if number != self._last_countdown_number:
                    self._last_countdown_number = number
                    self._play_countdown_tick()
            else:
                self._overlay(t("go"), TEAL, 60)

            self._draw_countdown_progress(
                min(1.0, elapsed / max(0.001, duration))
            )
        except Exception:
            traceback.print_exc()
        ...
        if elapsed >= duration:
            self._countdown_job = None
            self._draw_countdown_progress(1.0)

            # Keep GO visible for a full second. The distinctive audio cue
            # is played by _begin_record(), immediately before mic.start().
            self._go_job = self.after(
                int(GO_DELAY * 1000),
                self._begin_record
            )
            return

        # Smooth visual update (~60 FPS); timing remains anchored to t0.
        self._countdown_job = self.after(16, self._countdown)

    def _play_countdown_tick(self):
        """Short, non-blocking cue for each countdown number."""
        if HAVE_WINSOUND:
            def beep():
                try:
                    winsound.Beep(720, 55)
                except Exception:
                    pass
    ...
    def _play_start_cue(self):
        """Distinct two-tone cue marking the actual recording start."""
        if HAVE_WINSOUND:
            def beep():
                try:
                    winsound.Beep(1100, 120)
                    time.sleep(0.025)
                    winsound.Beep(1450, 150)
                except Exception:
                    pass
            ...
            try:
                self.bell()
                self.after(100, self.bell)
            except Exception:
                pass

    def _draw_countdown_progress(self, progress):
        """
        Draw/update the countdown progress bar inside the video overlay.

        progress: 0.0 -> 1.0
        """
        if not hasattr(self, "video_box"):
            return

        vx, vy, vw, vh = self.video_box
        progress = max(0.0, min(1.0, float(progress)))

        bar_w = min(700, max(260, vw - 80))
        bar_h = 10
        x0 = vx + (vw - bar_w) / 2
        y0 = vy + vh - 34

        if self.countdown_bar_bg is None:
            self.countdown_bar_bg = self.cv.create_rectangle(
                x0, y0, x0 + bar_w, y0 + bar_h,
                fill="#252942", outline="", state="normal"
            )
            self.countdown_bar = self.cv.create_rectangle(
                x0, y0, x0, y0 + bar_h,
                fill=TEAL, outline="", state="normal"
            )
        else:
            self.cv.coords(
                self.countdown_bar_bg,
                x0, y0, x0 + bar_w, y0 + bar_h
            )
            self.cv.itemconfigure(
                self.countdown_bar_bg, state="normal"
            )
            self.cv.itemconfigure(
                self.countdown_bar, state="normal"
            )
        ...
        self.cv.tag_raise(self.countdown_bar_bg)
        self.cv.tag_raise(self.countdown_bar)

    def _begin_record(self):
        if self.phase != "countdown":
            return

        if self._go_job is not None:
            try:
                self.after_cancel(self._go_job)
            except Exception:
                pass
            self._go_job = None
        ...
        if getattr(self.pack, "backing_audio", None) is not None:
            playback = ds.slice_audio(
                self.pack.backing_audio,
                line.start,
                dur
            )

        # This cue is deliberately not sent through self.mic.play().
        # That avoids restarting/sharing the audio stream at the recording
        # transition.
        ...
        # Establish the video timeline only after the input stream is live.
        ...
        self._play = {
            ...
            "dur": float(dur),
            "t0": record_t0,
            "cb": self._finish_record
        }
        self._frame_tick()

        # Safety net in case the frame loop does not reach the end.
        self._rec_guard = self.after(
            int(dur * 1000) + 500,
            self._finish_record
        )
    ...
    def _finish_record(self):
        ...
        try:
            ...
            if self.countdown_bar_bg is not None:
                self.cv.itemconfigure(
                    self.countdown_bar_bg, state="hidden"
                )
            if self.countdown_bar is not None:
                self.cv.itemconfigure(
                    self.countdown_bar, state="hidden"
                )
            self.sync()
        except Exception:
            traceback.print_exc()
